...
<div class="container">
  ...
  <div class="tabs">
    ...
    <button class="tab" onclick="switchTab('voice')">&#x1F399;&#xFE0F; Voz</button>
  </div>
  ...
  <div id="panel-voice" class="panel">
    <div class="card" style="text-align:center">
      <div class="card-title">&#x1F399;&#xFE0F; Control por Voz</div>
      <p style="font-size:0.85rem;color:var(--muted);margin-bottom:20px">Dile comandos como "adelante", "atrás", "izquierda", "derecha" o "parar".</p>
      <div style="position:relative;width:100px;height:100px;margin:0 auto">
        <div id="v-pulse" style="position:absolute;inset:0;border-radius:50%;background:var(--cyan);opacity:0;transition:all 0.3s"></div>
        <button id="v-btn" onclick="toggleVoice()" style="position:relative;width:100%;height:100%;border-radius:50%;border:none;background:var(--card);border:2px solid var(--border);color:var(--cyan);font-size:2.5rem;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all 0.3s">
          &#x1F399;&#xFE0F;
        </button>
      </div>
      <div id="v-status" style="margin-top:20px;font-weight:600;color:var(--cyan);height:24px">Esperando...</div>
      <div id="v-result" style="margin-top:10px;font-size:1.1rem;font-style:italic;color:var(--muted);min-height:30px"></div>
    </div>
  </div>
...
<script>
  const TABS = {robot:0, gps:1, nav:2, individual:3, voice:4};
  let autoMode = false;
  ...
  if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
    ...
    recognition.onstart = () => {
      recognizing = true;
      document.getElementById('v-btn').style.borderColor = 'var(--cyan)';
      document.getElementById('v-pulse').style.opacity = '0.2';
      document.getElementById('v-pulse').style.transform = 'scale(1.3)';
      document.getElementById('v-pulse').style.animation = 'pulse 1.5s infinite';
      document.getElementById('v-status').textContent = 'Escuchando...';
    };

    recognition.onend = () => {
      recognizing = false;
      document.getElementById('v-btn').style.borderColor = 'var(--border)';
      document.getElementById('v-pulse').style.opacity = '0';
      document.getElementById('v-pulse').style.animation = 'none';
      document.getElementById('v-status').textContent = 'Micrófono apagado';
      if (document.getElementById('panel-voice').classList.contains('active')) {
        // Reiniciar si el panel sigue activo para mantener escucha continua
        // recognition.start(); // Opcional, mejor dejar que el usuario lo active
      }
    };

    recognition.onresult = (event) => {
      const result = event.results[event.results.length - 1][0].transcript.toLowerCase().trim();
      document.getElementById('v-result').textContent = '"' + result + '"';
      
      if (result.includes('adelante') || result.includes('avanza')) cmd('avanzar');
      else if (result.includes('atrás') || result.includes('retrocede')) cmd('retroceder');
      else if (result.includes('izquierda')) cmd('izquierda');
      else if (result.includes('derecha')) cmd('derecha');
      else if (result.includes('para') || result.includes('detén') || result.includes('stop')) cmd('parar');
    };
  }
